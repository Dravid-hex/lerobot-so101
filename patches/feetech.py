...
def patch_setPacketTimeout(self, packet_length):  # noqa: N802
    ...
    self.packet_timeout = (self.tx_time_per_byte * packet_length) + (self.tx_time_per_byte * 3.0) + 200
...
class FeetechMotorsBus(SerialMotorsBus):
    ...
    def _is_error(self, error: int) -> bool:
        # Ignore bit 0 (voltage error) always set on 12V motors
        return (error & ~0x01) != self._no_error
    ...
    def write_calibration(self, calibration_dict: dict[str, MotorCalibration], cache: bool = True) -> None:
        for motor, calibration in calibration_dict.items():
            if self.protocol_version == 0:
                if "wrist_roll" not in str(motor):
